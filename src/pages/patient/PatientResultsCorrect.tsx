...
export type PatientResultsCorrectProps = {
  patientId: string;
  onNavigateBack?: () => void;
  onClose?: () => void;
  onSeeScan?: () => void;
...
/**
 * PatientResultsCorrect Page Component
 * 
 * Generic correct answer results page for any patient.
 * Displays results when user selects all correct answers.
 * 
 ...
 */
export default function PatientResultsCorrect({
  patientId,
  onNavigateBack,
  onClose,
  onSeeScan,
}: PatientResultsCorrectProps) {
  ...
  const resultData = results.correct;
  ...
  return (
    <>
      <QuizPageLayout
        ...
        onClose={onClose}
        footer={footer}
        aria-label={`${config.name} quiz correct results page`}
      >
        <TwoColumnContent
